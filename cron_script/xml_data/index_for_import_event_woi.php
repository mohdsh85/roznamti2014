...
$folderPath='feedsShofeeTv/'.$date.'/xmlFiles/';
		...
		//print_r($content);
		$arrayChannels=array('ME MBC1.xml','WOA MBC2.xml');
...
{
	if(in_array($content[$j],$arrayChannels))
		{
					//$dom = DOMDocument::load('../../../cron_scripts/xml_data/shoofeetv.xml') or die('Eror:');
					$channelIdParserr=0;
					$channelId=0;
					$dom = DOMDocument::load($folderPath.$content[$j]) or die('Eror:');
					...
					foreach ($params as $param) //go to each section 1 by 1
					{
						...
						//	echo $progTitleen;
							...
							{
								$randomMappedKey=time()+rand(1,1000);
								$values=array('start_date'=>$startTime,'end_date'=>$endTime,'owner'=>'1','cat_id'=>'9','url'=>$url,'other_id'=>$channelId,'other_id_2'=>'0','event_mapped_key'=>$randomMappedKey);
